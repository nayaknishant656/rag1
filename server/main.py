...
async def ask_question(question: str = Form(...)):
    try:
        ...
        from modules.embedding_utils import embed_query_safe
        ...
        # 2. Embed the question safely with retry logic
        embedded_query = embed_query_safe(embed_model, question)
        ...
        for match in res.get("matches", []):
            ...
            if text:
                ...
                logger.warning(f"Pinecone vector match {match.get('id')} has no 'text' field in metadata.")

        logger.info(f"Retrieved {len(docs)} valid context documents for user query.")
